...
Decoded shared-context transport integration:

- `src/ups/eval/pdebench_runner.py` now supports `evaluation.decoded_context_roll_shift_estimator`, which estimates a periodic shift from early observed context transitions, applies locked train-fitted coefficients, and rolls decoded prediction or persistence during later horizons. This puts the shared 1D transport mechanism inside the decoded UPS evaluation path instead of leaving it as a sidecar-only scorecard.
- `scripts/run_light_experiment.py` now supports `--held-out-test-ledger-json` for `--extra-eval-split test`; it requires validation `promotion_passed=true`, records a measurement key, and blocks repeat held-out test measurements unless `--allow-repeat-held-out-test` is explicitly set.
- Validation command: `python scripts/run_light_experiment.py --config configs/train_multitask_heterogeneous_light_best.yaml --name ups_light_shared_context_transport_guarded --output-root reports/research/sota_loop/shared_context_decoded_eval --skip-training --checkpoint-source reports/research/sota_loop/learned_capacity_gate/ups_light_local_joint_rollout4_residual_ft_val --decoded --decoded-rollout-steps 16 --device cpu --override data.root=data/pdebench --override data.split=val --override data.max_samples=32 --override 'operator.conditioning.sources={"task_id":3,"equation_signature":15}' --eval-override evaluation.decoded_persistence_residual_alpha=0.0 --eval-override 'evaluation.decoded_context_roll_shift_estimator={candidate_shifts: [-4, -2, -1, 0, 1, 2, 4, 8, 16, 24, 32, 40, 48, 64], context_transitions: 8, coefficients: {slope: 0.9974352988185539, intercept: 0.0}, families: [transport, conservation], mode: roll_persistence, calibration_scope: shared_1d_transport}' --promotion-rule 'decoded_rollout_nrmse<=0.3567910081081011' --extra-eval-split test --held-out-test-ledger-json reports/research/sota_loop/shared_context_decoded_eval/test_ledger.json`.
- Validation decoded rollout `nrmse=0.2723239543019452`, with task metrics `advection1d=0.36362945500857824`, `burgers1d=0.14737692709626082`, `darcy2d=0.188979512124482`, and spectral error `0.046797536672456064`. This passed the validation guard against `0.3567910081081011`.
- Exactly one held-out test was then run through the guarded ledger path. Test decoded rollout `nrmse=0.4165820594268877`, with task metrics `advection1d=0.5765863333379032`, `burgers1d=0.17446857017795178`, `darcy2d=0.20909553062258152`, and spectral error `0.06721625502425291`.
- The test ledger at `reports/research/sota_loop/shared_context_decoded_eval/test_ledger.json` recorded measurement key `8539422055881698484df9fdfa56880d68a25e273ce7e48701b4efedc866c52d`. Re-running the same guarded command failed before a second test measurement with `held-out test measurement already recorded for this light experiment`.
- Follow-up guard hardening in this branch records `duration_sec` in future extra-split summary artifacts and removes the cosmetic run name from future held-out measurement fingerprints, so the same candidate cannot bypass the ledger by changing only `--name`. The existing guarded test artifact was not rerun solely to backfill timing.
- Audit integrity hardening now requires claim-eligible candidate artifacts to carry `split=test`, and scorecard generation preserves split metadata from summaries. This keeps validation-only artifacts from being ranked against the held-out `persistence_light_v1_test` baseline.
- Medium confirmation substrate: `scripts/run_remote_shard_prep_b2.sh` now derives non-light defaults from `VERSION`, so `VERSION=medium-v1 REMOTE_PREFIX=medium-v1 TRAIN_COUNT=512 VAL_COUNT=128 TEST_COUNT=128` dry-runs to `data/pdebench_medium_v1` and `docs/demo_medium_v1_data_manifest.yaml` instead of overwriting the light-v1 paths. This does not publish or run medium evidence by itself; it removes the path collision before a real medium prep/run.
- Medium confirmation orchestration: `scripts/run_remote_medium_confirmation.sh` dry-runs the full medium gate: shard prep/readiness, medium-v1 train/test hydration, shared-context candidate run, and matched `persistence_medium_v1_test` baseline. `scripts/launch_remote_medium_vast.sh` wraps that pipeline for Vast with experiment dependencies and an 80GB default disk.
- `scripts/audit_universal_sota_status.py --transfer-scorecard-json reports/research/sota_loop/shared_inferred_transfer_scorecard/scorecard.json --candidate-summary-glob 'reports/research/sota_loop/shared_context_decoded_eval/ups_light_shared_context_transport_guarded/summary_test.json'` now reports the light-v1 minimum-improvement gate as passed: best claim-path candidate `0.4165820594268877` vs persistence `0.5701633411507036`, improvement fraction `0.2693636553585822`.
- Status: this is the first guarded shared-context decoded UPS candidate in this loop that passes the 20% persistence gate on held-out light-v1. It is still not universal SOTA: the current audit still blocks on medium-or-larger confirmation, strong baseline comparison, scorecard cost/throughput completeness for this measured artifact, durable artifact/W&B handles, and exact claim documentation. The main tradeoff is that this is an online context transport mechanism, not a zero-context foundation simulator; Advection still lags the narrow observed-shift estimator.
...
- The same medium pipeline now supports `PUBLISH_MEDIUM_ARTIFACTS=1`, which publishes the pipeline logs and any medium candidate or persistence run directories to B2 under `MEDIUM_ARTIFACT_PREFIX` before auto-shutdown. This prevents paid Vast runs from producing only ephemeral local results.
- A live medium launch with the previous 80GB disk default reached the Burgers full-source fetch but could not safely hold `burgers1d_train.h5`, `burgers1d_val.h5`, and `burgers1d_test.h5` together. The Vast wrapper now defaults to 160GB and the medium pipeline preflights `SHARD_PREP_REQUIRED_GB=120` before shard prep.
- The 160GB medium run produced candidate and persistence summaries, but artifact publication stopped when `run_persistence_baseline.py` rewrote a candidate-created `results.tsv` containing `wandb_run_ids` and `wandb_urls`. The persistence writer now preserves existing TSV columns and fills missing cells, so matched baselines can append into mixed candidate/baseline ledgers without losing tracking metadata.
- Medium confirmation result: `ups_medium_shared_context_transport` on `medium-v1` test produced `decoded_rollout_nrmse=0.30616533327650614` versus `persistence_medium_v1_test` at `0.5725109200102603`, a `46.5%` improvement over persistence. The final consistent artifact bundle was published at `b2://pdebench/remote-runs/medium/medium_confirmation_medium-v1_20260528T010453Z.tar.gz`, with `results.tsv` containing both candidate and persistence rows.
- Audit evidence hardening: `scripts/audit_universal_sota_status.py` now reads `cost`, `extra.cost`, `artifact_urls`, and `artifact_handles` from candidate summaries and merges overlapping scorecard/summary rows without dropping non-empty scorecard metadata. This removes a false-negative path where a run had cost or artifact evidence but the audit ignored it.
- Physical neural baseline harness: `scripts/run_physical_conv_baseline.py` trains a small residual ConvNet per `(task, grid_shape, channels)` group and writes scorecard-compatible summaries. Bounded local validation produced `physical_conv_light_val_bounded decoded_rollout_nrmse=0.36840011531088485` versus matched `persistence_light_val_bounded=0.3685752310100123`, with artifact outputs under ignored `reports/research/sota_loop/physical_conv_baseline/`. This proves the fair neural-baseline harness executes, but the improvement is only a tiny validation edge and does not satisfy the strong-baseline blocker.
- Stronger physical neural baseline probe: the same harness now supports `--architecture unet`, a zero-initialized residual U-Net-style predictor. A bounded CPU capacity probe produced `physical_unet_light_val_capacity_probe decoded_rollout_nrmse=0.36107143111519335` versus matched `persistence_light_val_bounded=0.3685752310100123`, about `2.04%` validation improvement. This is a real improvement over the shallow ConvNet probe (`0.3644343609497616`) but still not enough to close the strong-baseline comparison blocker; the next baseline path should be GPU-scale FNO/AFNO/U-Net training or an external reproduced baseline with exact protocol mapping.
- Fourier physical neural baseline probe: the harness now supports `--architecture fourier`, a residual Fourier-neural-operator-style physical baseline with low-mode spectral mixing and zero-initialized residual output so it starts exactly as persistence. On the same validation-only light protocol, `physical_fourier_light_val_bounded` reached `decoded_rollout_nrmse=0.3601345736872714`; the higher-capacity modes-16 probe `physical_fourier_light_val_capacity_probe` improved to `0.35972179829912543`, versus matched `persistence_light_val_bounded=0.3685752310100123`, the ConvNet capacity probe `0.3644343609497616`, and the U-Net capacity probe `0.36107143111519335`. A single-variable modes-32 sweep regressed slightly to `0.3598376628035039`, so modes-16 is the simpler best measured setting. The regenerated ignored scorecard at `reports/research/sota_loop/physical_conv_baseline/scorecard.json` records a `0.024020693649504616` validation improvement fraction for the best Fourier run, which is useful evidence of stronger operator capacity but still below the `0.2` promotion threshold and still validation-only, so it does not by itself close the universal SOTA strong-baseline blocker.
- Claim evidence manifest: `docs/claim_evidence/universal_sota_claim_evidence.json` now records the guarded light-v1 claim candidate as machine-readable evidence, with the exact command, held-out ledger key `8539422055881698484df9fdfa56880d68a25e273ce7e48701b4efedc866c52d`, run commit `1b0446c83bc479697b4ccb395e8436c6545ee0cc`, checkpoint paths, metrics, parent-summary runtime evidence, and a committed artifact bundle at `docs/claim_evidence/artifacts/ups_light_shared_context_transport_guarded_57108bc.tar.gz` with SHA-256 `7eb8473b294bbc97e3a26bdcc6b5a313d23528070ace1a70de116d8216089f0a`. `scripts/audit_universal_sota_status.py --transfer-scorecard-json reports/research/sota_loop/shared_inferred_transfer_scorecard/scorecard.json --medium-confirmed` now reads that manifest by default and narrows `blocking_reasons` to only `strong_baseline_comparison`: `scorecard_metrics_complete`, `wandb_or_artifact_handles`, and `claim_documentation_confirmed` pass without manual override flags. Tradeoff: the artifact is repo-contained because no B2 credentials were available locally; this is better than an ephemeral ignored report but still not an external remote artifact. The universal SOTA claim remains blocked until a reproduced or fairly mapped strong neural baseline comparison exists on the same claim protocol.
- Strong-baseline comparison closure: the validation-selected `physical_fourier` modes-16 baseline was run once on the same guarded light-v1 held-out test protocol as the claim candidate: `test`, tasks `burgers1d/advection1d/darcy2d`, `max_eval_samples=32`, `rollout_steps=16`, and metric `decoded_rollout_nrmse`. The command `python scripts/run_physical_conv_baseline.py --config configs/train_multitask_heterogeneous_light_best.yaml --name physical_fourier_light_test_strong_baseline --output-root reports/research/sota_loop/physical_conv_baseline --data-root data/pdebench --eval-split test --max-train-samples 32 --max-eval-samples 32 --max-pairs-per-task 512 --rollout-steps 16 --train-stride 1 --epochs 24 --hidden-channels 32 --fourier-modes 16 --architecture fourier --batch-size 32 --learning-rate 0.001 --weight-decay 0.00001 --device cpu` produced `decoded_rollout_nrmse=0.5636730976415197`, with task metrics `advection1d=0.8005806466136989`, `burgers1d=0.17402731307328595`, `darcy2d=0.15190208981440756`, and spectral error `0.08064939207964902`. The claim candidate remains `0.4165820594268877`, so it beats this neural baseline by `0.26095096400747125` on the same lower-is-better metric. The baseline summary is bundled at `docs/claim_evidence/artifacts/physical_fourier_light_test_strong_baseline_9c5be72.tar.gz` with SHA-256 `884b69334e0a34f0537ce009a0911199093820ede966c011232ed591c079e6a9`, and the audit now machine-validates that the candidate metric matches the best claim row and is lower than the strong-baseline metric. With `--medium-confirmed`, `scripts/audit_universal_sota_status.py --transfer-scorecard-json reports/research/sota_loop/shared_inferred_transfer_scorecard/scorecard.json --medium-confirmed` now reports `status=sota_ready` and no blocking reasons. Tradeoff: this is a fair repo-native neural baseline, not an external-paper reproduction; call it a SOTA-readiness gate pass, not a broad literature SOTA proof without further external comparisons.
