...
PLAN=${PLAN:-docs/research/artifacts/strat_v1_darcy_fno_regime_balanced_objective_plan.json}
...
SCRATCH_ROOT=${SCRATCH_ROOT:-reports/research/darcy_regime_balanced_scratch}
CACHE=${CACHE:-$SCRATCH_ROOT/cache}
DATA_ROOT=${DATA_ROOT:-$SCRATCH_ROOT/data}
OUTPUT_DIR=${OUTPUT_DIR:-reports/research/darcy_fno_regime_balanced_objective}
STAGE_REPORT=${STAGE_REPORT:-reports/research/darcy_fno_regime_balanced_stage.json}
RESULT=${RESULT:-reports/research/darcy_fno_regime_balanced_result.json}
RUNNER_LOG=${RUNNER_LOG:-$SCRATCH_ROOT/remote_runner.log}
ARTIFACT_PREFIX=${ARTIFACT_PREFIX:-remote-runs/darcy-fno-regime-balanced-objective}
...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "DRY RUN: validation-only Darcy R-mean/B-minimax D3; held-out data is inaccessible."
  echo "$PYTHON -m ups.data.cli stage --lock $TRAINING_LOCK --cache $CACHE --run-dir $DATA_ROOT --report $STAGE_REPORT"
  echo "$PYTHON executes the exact command bound by $PLAN, adding --resume only after plan-bound checkpoint hydration."
  echo "$PYTHON scripts/materialize_darcy_fno_regime_balanced_objective.py --plan $PLAN --summary $OUTPUT_DIR/summary.json --output $RESULT"
  echo "On failure, checkpoint state is synchronized to a temporary plan-bound B2 prefix; immutable success deletes it."
  exit 0
fi
...
if p.get("schema_version") != 2: raise SystemExit("D3 plan schema version must be 2")
if p.get("mode") != "validation_only" or p.get("heldout_access") != "forbidden" or p.get("measurement_lock_access") != "forbidden": raise SystemExit("refusing plan without complete validation-only access restrictions")
v=p.get("plan_sha256")
if v != canonical({k: value for k, value in p.items() if k != "plan_sha256"}): raise SystemExit("invalid canonical plan self hash")
command=p.get("command")
if not isinstance(command,list) or p.get("command_sha256") != canonical(command): raise SystemExit("invalid canonical plan command hash")
if any("test" in str(item).lower() for item in command): raise SystemExit("refusing test-capable command")
print(v)
PY
)
...
preserve_resume() {
  ...
  if [ "$success" -ne 1 ] && [ -d "$OUTPUT_DIR" ]; then
    ...
    echo "Preserved resumable D3 checkpoints: b2://${B2_BUCKET}/${temporary_prefix}/output" >&2
  fi
  exit "$status"
}
...
[ "$(sha256sum "$PLAN" | awk '{print $1}')" = "$plan_file_sha" ] || {
  echo "D3 plan bytes changed after validation" >&2
  exit 1
}
...
if current != plan["bindings"]["source"]["implementation_commit"] and subprocess.run(["git","merge-base","--is-ancestor",plan["bindings"]["source"]["implementation_commit"],current]).returncode: raise SystemExit("implementation commit is not in remote HEAD")
...
if not isinstance(command,list) or any("test" in str(x).lower() for x in command): raise SystemExit("refusing ambiguous or test-capable command")
if len(sys.argv) > 2: command.append("--resume")
subprocess.run(command, check=True)
PY
cp "$RUNNER_LOG" "$OUTPUT_DIR/remote_runner.log"

$PYTHON scripts/materialize_darcy_fno_regime_balanced_objective.py --plan "$PLAN" --summary "$OUTPUT_DIR/summary.json" --output "$RESULT"
stamp=$(date -u +%Y%m%dT%H%M%SZ)
archive_name="darcy_fno_regime_balanced_objective_${stamp}.tar.gz"
...
[ "$remote_digest" = "$digest" ] || {
  echo "Immutable D3 artifact read-back hash mismatch" >&2
  exit 1
}
rclone purge "UPSB2:${B2_BUCKET}/${temporary_prefix}" >/dev/null 2>&1 || true
success=1
trap - EXIT
echo "Published immutable Darcy D3 artifact: b2://${B2_BUCKET}/${remote_key}"
