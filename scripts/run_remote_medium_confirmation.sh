...
SHARD_PREP_REQUIRED_GB=${SHARD_PREP_REQUIRED_GB:-120}
CHECK_B2=${CHECK_B2:-1}
...
if [ "$shards_ready" -ne 1 ] && [ "$PREP_SHARDS" -eq 1 ]; then
  ...
  echo "Medium shard prep assignments: VERSION=${VERSION} REMOTE_PREFIX=${REMOTE_PREFIX} REMOTE_B2_PREFIX=${REMOTE_PREFIX} TRAIN_COUNT=${TRAIN_COUNT} VAL_COUNT=${EVAL_COUNT} TEST_COUNT=${EVAL_COUNT} SHARD_PREP_REQUIRED_GB=${SHARD_PREP_REQUIRED_GB}"
  (
    DRY_RUN="$DRY_RUN" \
      ENV_FILE="$ENV_FILE" \
      VERSION="$VERSION" \
