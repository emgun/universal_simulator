...
def build_burgers_full(raw_root: Path, out_root: Path, artifacts_dir: Path) -> Path:
    ...
    if not _try_run(
        [
            "python",
            "scripts/convert_pdebench.py",
            "--pattern",
            str(raw_root / "1D/Burgers/Valid/*.hdf5"),
            "--out",
            str(out_root / "burgers1d_val.h5"),
        ]
    ):
    ...
    if not _try_run(
        [
            ...
            "scripts/convert_pdebench.py",
            "--pattern",
            str(raw_root / "1D/Burgers/Test/*.hdf5"),
            "--out",
            str(out_root / "burgers1d_test.h5"),
        ]
    ):
...
def build_advection_full(raw_root: Path, out_root: Path, artifacts_dir: Path) -> Path:
    ...
    if not _try_run(
        [
            ...
            "scripts/convert_pdebench.py",
            "--pattern",
            str(raw_root / "1D/Advection/Valid/*.hdf5"),
            "--out",
            str(out_root / "advection1d_val.h5"),
        ]
    ):
    ...
    if not _try_run(
        [
            ...
            "scripts/convert_pdebench.py",
            "--pattern",
            str(raw_root / "1D/Advection/Test/*.hdf5"),
            "--out",
            str(out_root / "advection1d_test.h5"),
        ]
    ):
        src = out_root / "advection1d_train.h5"
...
def main() -> None:
    ...
    print("Upload with:")
    print(
        f"python scripts/upload_artifact.py {name} dataset {path} --project universal-simulator --entity YOUR_ENTITY"
    )
