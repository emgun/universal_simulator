    ...
    *,
    ...
    if receipt_path is not None:
        ...
        if receipt.get("status") != "succeeded" or receipt.get("destroyed") is not True:
            raise ValueError("refusing D5 finalization without a succeeded destroyed Vast receipt")
    ...
    try:
        ...
        if not _HEX_64.fullmatch(digest):
            raise ValueError("D5 ingress digest sidecar is malformed")
        source_digest, source_size = _stream_sha256(
            client.get_object(Bucket=bucket, Key=success_key)["Body"]
        )
        if source_size <= 0:
            raise ValueError("D5 ingress artifact is empty")
        if source_digest != digest:
            raise ValueError("D5 ingress artifact differs from its digest sidecar")
        archive_name = f"strat_v1_shared_tier_b_{manifest['launch_id']}.tar.gz"
        ...
        if destination_size != source_size:
            raise ValueError("immutable D5 server-side copy size mismatch")
        if destination_digest != digest:
            raise ValueError("immutable D5 artifact read-back mismatch")
    except ValueError:
        raise
    except Exception:
        raise RuntimeError("D5 local finalization failed; ingress objects were retained") from None
...
def main() -> None:
    ...
    print(
        "Published immutable D5 artifact: "
        f"{finalize(args.manifest, env_file=args.env_file, receipt_path=args.receipt)}"
    )
