...
"""Validate validation-only UPS advection horizon-weighted candidate evidence."""
...
DEFAULT_EVIDENCE_JSON = (
    "docs/claim_evidence/ups_advection_horizon_weighted_candidate_val_evidence.json"
)
EXPECTED_MEASUREMENT_TYPE = "ups_advection_horizon_weighted_training_candidate_validation"
EXPECTED_DECISION_STATUS = "phase_gate_not_cleared_horizon_weighted_candidate"
METRICS_TO_RECORD = (
...
def validate_evidence(evidence: Mapping[str, Any], *, root: Path | None = None) -> list[str]:
    ...
    if code_change.get("new_stage_config_key") != "rollout_loss_horizon_power":
        errors.append("code_change.new_stage_config_key must be rollout_loss_horizon_power")
    ...
    comparison = _as_mapping(
        evidence.get("comparison_to_rollout16_candidate"),
        "comparison_to_rollout16_candidate",
        errors,
    )
    if comparison.get("overall_absolute_improvement", 0.0) <= 0.0:
        errors.append(
            "comparison_to_rollout16_candidate.overall_absolute_improvement must be positive"
        )
    if comparison.get("advection_h16_absolute_improvement", 0.0) <= 0.0:
        errors.append(
            "comparison_to_rollout16_candidate.advection_h16_absolute_improvement positive"
        )
