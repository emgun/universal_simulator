...
OFFER_ID=${OFFER_ID:-}
REMOTE_SCRIPT=${REMOTE_SCRIPT:-scripts/run_remote_darcy_fno_regime_balanced_objective.sh}
...
[[ "$GIT_REF" =~ ^[0-9a-f]{40}$ ]] || { echo "GIT_REF must be a full lowercase 40-character commit" >&2; exit 2; }
...
if [ "$DRY_RUN" -eq 0 ]; then
  ...
  git ls-remote origin | awk -v commit="$GIT_REF" '$1 == commit {found=1} END {exit !found}' || { echo "GIT_REF must be the exact commit of a pushed remote ref" >&2; exit 2; }
