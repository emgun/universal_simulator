...
PLAN=${PLAN:-docs/research/artifacts/strat_v1_darcy_fno_affine_head_ablation_plan.json}
...
SCRATCH_ROOT=${SCRATCH_ROOT:-reports/research/darcy_affine_head_scratch}
...
if [ "$DRY_RUN" -eq 1 ]; then
  echo "DRY RUN: validation-only Darcy K-long/A-affine D2; held-out data is inaccessible."
  ...
  echo "$PYTHON scripts/materialize_darcy_fno_affine_head_ablation.py --plan $PLAN --summary $OUTPUT_DIR/summary.json --output $RESULT"
...
if p.get("schema_version") != 2: raise SystemExit("D2 plan schema version must be 2")
...
[ "$(sha256sum "$PLAN" | awk '{print $1}')" = "$plan_file_sha" ] || {
  echo "D2 plan bytes changed after validation" >&2
  exit 1
}
...
archive_name="darcy_fno_affine_head_ablation_${stamp}.tar.gz"
...
success=1
trap - EXIT
echo "Published immutable Darcy D2 artifact: b2://${B2_BUCKET}/${remote_key}"
