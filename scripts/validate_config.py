    ...
    if cfg.get("ttc", {}).get("enabled"):
        checks.append(
            (
                ...
                (
                    f"{ttc_decoder_latent} == {latent_dim}"
                    if ttc_decoder_latent
                    else "Missing ttc.decoder.latent_dim"
                ),
            )
        )
        ...
        checks.append(
            (
                ...
                (
                    f"{ttc_decoder_hidden} == {op_hidden}"
                    if ttc_decoder_hidden
                    else "Missing ttc.decoder.hidden_dim"
                ),
            )
        )
...
def validate_stages(cfg: Dict) -> List[Tuple[str, bool, str]]:
    ...
    checks.append(("operator.epochs > 0", op_epochs > 0, f"epochs = {op_epochs}"))
...
def validate_evaluation(cfg: Dict) -> List[Tuple[str, bool, str]]:
    ...
    checks.append(("evaluation.enabled is true", eval_enabled is True, f"enabled = {eval_enabled}"))
...
def validate_data(cfg: Dict) -> List[Tuple[str, bool, str]]:
    ...
    checks.append(
        ("data.task defined", task is not None, f"task = {task}" if task else "Missing data.task")
    )
    ...
    checks.append(
        ("data.split is valid", split in valid_splits, f"split = {split} (valid: {valid_splits})")
    )
    ...
    if root:
        ...
        checks.append(("data.root defined", False, "Missing data.root"))
...
def validate_cache(cfg: Dict) -> List[Tuple[str, bool, str]]:
    ...
    try:
        ...
        checks.append(
            ("cache metadata valid", True, f"hash={metadata.get('config_hash', 'unknown')}")
        )
        ...
        checks.append(("cache metadata parseable", False, f"Corrupted metadata: {e}"))

    return checks


def validate_distributed_config(cfg: Dict) -> List[Tuple[str, bool, str]]:
    """Validate distributed training configuration."""
    checks = []

    num_gpus = cfg.get("training", {}).get("num_gpus", 1)

    # Check 1: num_gpus is valid
    checks.append(
        (
            "training.num_gpus >= 1",
            num_gpus >= 1,
            f"num_gpus = {num_gpus}" if num_gpus >= 1 else f"Invalid num_gpus = {num_gpus}",
        )
    )

    # Check 2: num_gpus not too large (diminishing returns)
    checks.append(
        (
            "training.num_gpus <= 8 (recommended)",
            num_gpus <= 8,
            (
                f"num_gpus = {num_gpus} (>8 has diminishing returns)"
                if num_gpus > 8
                else f"num_gpus = {num_gpus}"
            ),
        )
    )
    ...
    # Check 4: multi-task with DDP requires balanced strategy
    ...
    if isinstance(task, list) and len(task) > 1 and num_gpus > 1:
        ...
        checks.append(
            (
                "multi-task DDP uses task_sampling.strategy='balanced'",
                strategy == "balanced",
                (
                    f"task_sampling.strategy = '{strategy}' (should be 'balanced' for multi-task DDP)"
...
def main():
    ...
    distributed_checks = validate_distributed_config(cfg)
    print_results("Distributed Training", distributed_checks)
    all_checks.extend(distributed_checks)
