...
PASS_STATUSES_BY_MODE = {
    ...
    "observed-accepted": {"literal_achieved", "observed_context_achieved"},
}
...
def audit_objective(args: argparse.Namespace) -> dict[str, Any]:
    ...
    requirements = [
        {
            ...
            "status": "satisfied"
            if (constant_audit or observed_audit or feature_diag)
            else "missing",
            "evidence": ", ".join(path for path in (constant_path, observed_path, feature_path) if path)
        ...
        },
        {
            ...
            "status": "satisfied"
            if constant_status == "achieved" or (observed_accepted and observed_status == "achieved")
            else "blocked",
            "evidence": f"constant_audit_status={constant_status}; observed_audit_status={observed_status}",
        },
        {
    ...
    return {
        ...
        "recommendation": (
            "If two-frame observed context is benchmark-accepted, promote observed-context result; "
