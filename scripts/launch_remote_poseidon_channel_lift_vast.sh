...
# Dry-run-first Vast launcher for the P2.2 Poseidon channel-lift validation.
#
# Defaults do not launch paid compute. Set DRY_RUN=0 only after reviewing the
# generated Vast command/onstart and making sure GIT_REF points to a ref that
# contains scripts/run_remote_poseidon_channel_lift.sh.
...
PIPELINE_ROOT=${PIPELINE_ROOT:-reports/demo/remote_poseidon_channel_lift}
REMOTE_SCRIPT=${REMOTE_SCRIPT:-scripts/run_remote_poseidon_channel_lift.sh}
AUTO_SHUTDOWN=${AUTO_SHUTDOWN:-1}
SSH=${SSH:-1}
ARGS_MODE=${ARGS_MODE:-0}
INSTALL_MODE=${INSTALL_MODE:-experiment}
BOOTSTRAP_MODE=${BOOTSTRAP_MODE:-tracked-script}
...
if [ "$DRY_RUN" -eq 0 ] && [ "$ALLOW_MISSING_B2" -ne 1 ]; then
  ...
  if [ "${#missing_b2[@]}" -gt 0 ]; then
    echo "Refusing DRY_RUN=0: ${REMOTE_SCRIPT} expects B2 for train/val hydration and artifact publishing, but missing: ${missing_b2[*]}." >&2
...
if [ "$DRY_RUN" -eq 0 ]; then
  if ! git fetch --quiet origin "$GIT_REF"; then
    echo "Refusing DRY_RUN=0: could not fetch origin ref '${GIT_REF}'." >&2
    echo "Push a ref containing ${REMOTE_SCRIPT}, or set GIT_REF to one that already contains it." >&2
    exit 2
  fi
  if ! git cat-file -e "FETCH_HEAD:${REMOTE_SCRIPT}" 2>/dev/null; then
    echo "Refusing DRY_RUN=0: origin ref '${GIT_REF}' does not contain ${REMOTE_SCRIPT}." >&2
    echo "Push a ref containing the remote script before launching a disposable Vast instance." >&2
    exit 2
  fi
  if [ "$BOOTSTRAP_MODE" = "tracked-script" ] && ! git cat-file -e "FETCH_HEAD:scripts/vast_remote_bootstrap.sh" 2>/dev/null; then
    echo "Refusing DRY_RUN=0: origin ref '${GIT_REF}' does not contain scripts/vast_remote_bootstrap.sh." >&2
    exit 2
  fi
fi

script_args="DRY_RUN=0 ENV_FILE=.env PIPELINE_ROOT=$PIPELINE_ROOT PUBLISH_ARTIFACTS=1 $EXTRA_PIPELINE_ARGS"
