...
"""Pre-register the validation-only Darcy R-mean/B-minimax D3 experiment."""
...
D2_CONTROL_PRIMARY = 0.12244374883447355
SOURCE_FILES = (
    ...
    "scripts/plan_darcy_fno_regime_balanced_objective.py",
    "scripts/materialize_darcy_fno_regime_balanced_objective.py",
    "scripts/run_remote_darcy_fno_regime_balanced_objective.sh",
    "scripts/launch_darcy_fno_regime_balanced_objective_vast.sh",
...
def source_manifest(implementation_commit: str) -> dict[str, str]:
    ...
    if status.strip():
        raise ValueError(f"D3 source paths must be clean and tracked:\n{status.rstrip()}")
    ...
    for relative in source_paths():
        ...
        try:
            ...
            raise ValueError(
                f"D3 source is not tracked by implementation commit: {relative}"
            ) from exc
        if live != committed:
            raise ValueError(f"D3 source bytes differ from implementation commit: {relative}")
...
def main() -> None:
    ...
    if lock.lock_sha256 != LOCK_SHA256 or lock.purpose != "training":
        raise ValueError("D3 requires the frozen universal training lock")
    if set(lock.requested_roles) != {"train", "valid"} or any(
        x.role == "test" for x in lock.objects
    ):
        raise PermissionError("D3 must remain train/validation-only")
    observed = {
        x.object_id: x.checksums.get("sha256") for x in lock.objects if x.object_id in DARCY_OBJECTS
    }
    if observed != DARCY_OBJECTS:
        raise ValueError("Darcy object identities differ from the D3 contract")

    d2 = checked_artifact(args.d2_result, label="D2 result")
    d2_primary = float(d2.get("arms", {}).get("K-long", {}).get("primary_value", float("nan")))
    if d2_primary != D2_CONTROL_PRIMARY:
        raise ValueError("D2 direct conditioned control differs from the frozen value")
    missing = [relative for relative in source_paths() if not (REPO_ROOT / relative).is_file()]
    if missing:
        raise FileNotFoundError(f"D3 source files are missing: {missing}")
    ...
    plan = {
        "schema_version": 2,
        "plan_id": "strat-v1-darcy-fno-regime-balanced-objective-d3",
        "mode": "validation_only",
        "heldout_access": "forbidden",
        "measurement_lock_access": "forbidden",
        "question": "Does a matched minimax objective remove the remaining beta=100 Darcy error concentration?",
        "bindings": {
            "source": {
            ...
            "d2_result": {
                "path": str(args.d2_result),
                "file_sha256": file_sha256(args.d2_result),
                "artifact_sha256": d2["artifact_sha256"],
                "direct_conditioned_primary": D2_CONTROL_PRIMARY,
            },
        },
        ...
        "design": {
            ...
            "objectives": {
                "R-mean": {"mean_raw_solution_mse_weight": 1.0, "worst_regime_mse_weight": 0.0},
                "B-minimax": {
                    "mean_raw_solution_mse_weight": 0.5,
                    "worst_regime_mse_weight": 0.5,
                },
            },
            "single_continuous_trajectory_per_arm": True,
            "same_data_order_optimizer_and_updates": True,
            "R-mean": "direct beta-conditioned FNO with mean raw solution MSE",
            "B-minimax": "same FNO with 0.5 mean raw MSE plus 0.5 worst-regime raw MSE",
        },
        "gates": {
            "finite_all_primary_and_regime_metrics": True,
            "candidate_max_corrected_regime_spread_maximum": 1.5,
            "candidate_primary_maximum": D2_CONTROL_PRIMARY,
