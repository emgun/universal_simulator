...
def run_to_record(run: Any, *, metric_prefixes: Iterable[str] = ()) -> dict[str, Any]:
    ...
    record: dict[str, Any] = {
        ...
        "config_run_name": (
            config.get("logging", {}).get("wandb", {}).get("run_name", "")
    ...
    for key, value in summary.items():
        text_key = str(key)
        if (
            text_key.startswith("_")
            or not _scalar(value)
            or not _matches_prefix(text_key, metric_prefixes)
        ):
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Collect W&B run summaries into local registry artifacts"
    )
    parser.add_argument("--project", default=os.environ.get("WANDB_PROJECT", "universal-simulator"))
    parser.add_argument("--entity", default=os.environ.get("WANDB_ENTITY", ""))
    parser.add_argument("--limit", type=int, default=100)
    parser.add_argument(
        "--tag", action="append", default=[], help="Require this W&B tag; can be repeated"
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Metric prefix to retain; can be repeated",
    )
