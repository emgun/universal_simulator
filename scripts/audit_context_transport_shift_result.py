...
"""Audit the two-frame context transport-shift result artifact."""
...
def audit_context_result(args: argparse.Namespace) -> dict[str, Any]:
    gate, gate_path = _load_optional_json(args.context_gate_json)
    ...
    if gate is None:
        ...
        blockers = [f"missing context gate JSON: {args.context_gate_json}"]
        ...
        blockers = ["two-frame context transport validation failed the SOTA guard"]
        ...
        status = "blocked"
        blockers = ["two-frame context transport gate did not authorize held-out test"]
    ...
    return {
        ...
        "recommendation": (
            "Promote this result only if the benchmark policy accepts two initial context frames; "
            "otherwise keep it as evidence for a state-conditioned transport objective."
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Result from scripts/run_context_transport_shift_gate.py",
    )
