...
"""Independently verify D3 evidence and materialize its frozen gate result."""
...
def load_and_validate_plan(path: Path) -> dict[str, Any]:
    ...
    if plan.get("schema_version") != 2:
        raise ValueError("D3 plan schema version must be 2")
    if (
...
def main() -> None:
    ...
    control, candidate = arm_rows["R-mean"], arm_rows["B-minimax"]
    diagnostics = summary.get("diagnostics", {})
    shuffled = finite(
        diagnostics["deterministic_shuffled_beta"]["arms"]["B-minimax"][
            "relative_degradation_vs_true_beta"
        ],
        "candidate shuffled degradation",
    )
    sensitivity = finite(
        diagnostics["counterfactual_beta_sensitivity"]["B-minimax"][
    ...
    checks = {
        ...
        "candidate_primary_below_frozen_d2_control": candidate["primary_value"]
        ...
        "candidate_plateau_by_cap": candidate["plateau_epoch"] is not None
        and candidate["plateau_epoch"] <= gates["plateau"]["must_occur_by_epoch"],
    }
    result = {
        "schema_version": 2,
        "artifact_id": "strat-v1-darcy-fno-regime-balanced-objective-d3-result",
        ...
        "interpretation": (
            "regime_balanced_objective_validated"
            if all(checks.values())
            else "regime_balanced_objective_not_validated"
        ),
    }
