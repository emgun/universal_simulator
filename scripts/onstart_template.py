...
def generate_onstart_script(config: OnstartConfig) -> str:
    ...
    if config.train_config:
        lines.extend([
            ...
            "",
            "# Download full training data with retry logic",
            ...
            f'    if rclone copy B2TRAIN:{config.b2_bucket or "pdebench"}/full/burgers1d/burgers1d_train_000.h5 data/pdebench/ --progress --transfers=4 --checkers=8; then',
            ...
            "    echo 'Please check B2 credentials and network connectivity'",
            "    exit 1",
            "  fi",
            "  ",
            "  # Verify file integrity",
            "  if [ -f data/pdebench/burgers1d_train_000.h5 ]; then",
            "    FILE_SIZE=$(stat -f%z data/pdebench/burgers1d_train_000.h5 2>/dev/null || stat -c%s data/pdebench/burgers1d_train_000.h5 2>/dev/null)",
            "    if [ $FILE_SIZE -lt 1000000 ]; then",
            "      echo '❌ Downloaded file is too small (possibly corrupted)'",
            "      rm -f data/pdebench/burgers1d_train_000.h5",
            "      exit 1",
            "    fi",
            "    echo \"✅ File integrity check passed (${FILE_SIZE} bytes)\"",
            "  fi",
            "fi",
            "",
            "# Create symlink",
