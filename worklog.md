...
Rollout-stability recipe sweep live checkpoint (2026-06-11):
- Launched the P1.3 medium-v1 validation-only fixed-tier_b recipe sweep on Vast instance `40512322` after B2 credentials became available. The initial default `BATCH_SIZE=16` run produced repeated CUDA OOM skipped-batch warnings and was killed as invalid; the same instance was relaunched with `BATCH_SIZE=4`, preserving the model capacity contract while avoiding skipped-batch metrics.
- Remote hydration used only `burgers1d`, `advection1d`, and `darcy2d` train/val shards in `data/pdebench_medium_v1_runtime`; no test split files were observed and all completed summaries reported `val` split metadata with no task-root overrides.
- The valid batch-4 sweep has zero OOM warnings so far and five of six recipes completed. Completed validation `decoded_rollout_nrmse` values: `r_rollout8=0.8011641002837189`, `r_rollout16=0.7871739459416698`, `r_hpower=0.7620583413339258`, `r_semigroup=0.8024338475480444`, `r_long=0.838029201957462`.
- Interim decision state: best completed recipe is `r_hpower`, but it is still worse than tier_b capacity `0.7449043873888164` and far worse than persistence `0.38260034902058476`; no promotion or held-out test is justified from the partial result. Final `r_combo` is still running, and B2 publication plus `recipe_sweep_summary.json` verification remain pending.
- Later live check at 2026-06-11 14:54 UTC: `r_combo` had completed its 36-epoch operator stage and 12-epoch decoder stage, and was 2/18 epochs into operator-decoded with zero OOM warnings. No `recipe_sweep_summary.json` or B2 `remote-runs/recipe-sweep/` artifact existed yet.
- Follow-up live check at 2026-06-11 15:11 UTC: `r_combo` advanced to operator-decoded epoch 3/18 with epoch time about 516 seconds, still zero OOM warnings, still five recipe summaries, and still no pipeline self-summary or B2 artifact.
- Follow-up live check at 2026-06-11 15:22 UTC: `r_combo` advanced to operator-decoded epoch 4/18, still zero OOM warnings, still five recipe summaries, and still no pipeline self-summary. The final recipe is making forward progress but remains several hours from the summarize/publish step.
- Follow-up live check at 2026-06-11 15:32 UTC: `r_combo` advanced to operator-decoded epoch 6/18, still zero OOM warnings, still five recipe summaries, and still no pipeline self-summary.
- Follow-up live checks through 2026-06-11 16:44 UTC: `r_combo` advanced to operator-decoded epoch 14/18, still zero OOM warnings, still five recipe summaries, and still no pipeline self-summary. The run is healthy and compute-bound; artifact verification remains pending until the final recipe writes its summary and the runner publishes the tarball.
- Follow-up live check at 2026-06-11 16:52 UTC: `r_combo` advanced to operator-decoded epoch 15/18, still zero OOM warnings, still five recipe summaries, and still no pipeline self-summary.
- Follow-up live check at 2026-06-11 17:17 UTC: `r_combo` completed operator-decoded 18/18 and entered `joint_codec_operator`; joint epoch 0 completed in about 545 seconds, still zero OOM warnings, still five recipe summaries, and still no pipeline self-summary.

P1 recipe sweep complete (2026-06-11):
- Full six-recipe validation-only fixed-tier_b sweep completed at `BATCH_SIZE=4` with zero OOM warnings and no held-out/test data access. Published bundle verified from B2 at `b2://pdebench/remote-runs/recipe-sweep/recipe_sweep_medium-v1_20260611T185755Z.tar.gz`; local downloaded tarball SHA256 `521955d442c86c681cd5a19a30ff1ede197e24c08a15bee19b42eb6de86b3c93`.
- Durable self-summary committed at `docs/research/artifacts/p1_recipe_sweep_medium_v1_val.json`; human-readable result note added at `docs/research/2026-06-11-p1-recipe-sweep-results.md`.
...
P1 data-budget sweep live checkpoint (2026-06-11):
- Launched validation-only data-budget sweep on Vast instance `40615380` from branch `codex/p1-data-budget-sweep` after pushing the runner. Remote checkout landed at commit `846f60e`.
- Initial default `BATCH_SIZE=16` run was invalidated and stopped after `120` OOM skipped-batch warnings appeared; invalid output root `reports/data_budget_sweep_remote` contains only a non-authoritative `n128` summary and must not be used as evidence.
- Relaunched cleanly on the same instance at `BATCH_SIZE=4`, `OUTPUT_ROOT=reports/data_budget_sweep_remote_batch4`, `PIPELINE_ROOT=reports/demo/remote_data_budget_sweep_batch4`, and `FETCH_DATA=0` against the already hydrated train/val-only shards. Clean run has observed only `advection1d`, `burgers1d`, and `darcy2d` train/val files.
- First clean result: `ups_medium_data_budget_n128` validation `decoded_rollout_nrmse=0.7859101457888691`, `decoded_h16_nrmse=0.7918133526998239`, split `val`, no task-root override, OOM count `0`.
- Additional clean partials before interruption: `ups_medium_data_budget_n256` validation `decoded_rollout_nrmse=0.867498778805475`, `decoded_h16_nrmse=0.9656506265101972`; `ups_medium_data_budget_n512` validation `decoded_rollout_nrmse=0.8097381326995998`, `decoded_h16_nrmse=0.8256953247507288`. The `n1024` run was still in training and no `data_budget_sweep_summary.json` or B2 artifact had been produced.
- The clean batch-4 run was interrupted when instance `40615380` stopped mid-`n1024`, likely because the original auto-shutdown onstart path from the invalid batch-16 launch caught up after the detached rerun. Restart returned `Required resources are currently unavailable, state change queued.`
- Follow-up hardening committed on branch `codex/p1-data-budget-sweep` as `b704a5a`: `scripts/run_remote_data_budget_sweep.sh` now defaults to `BATCH_SIZE=4`, the contract records that safe default, and the unit test asserts the emitted training override.
- Destroyed abandoned stopped instance `40615380` after preserving the partial metrics above; unrelated stopped instance `40586170` was left untouched. A clean replacement launch with `AUTO_SHUTDOWN=0`, batch-4 roots, B2 artifact publishing, and pushed commit `b704a5a` passed dry-run generation but live provisioning failed before instance creation with `failed with error 400: Your account lacks credit; see the billing page.`
- Current state: data-budget sweep remains not completed and no held-out/test split was touched. Resume by funding Vast or using another GPU runner, then launch `ENV_FILE=/Users/emerygunselman/Code/universal_simulator/.env PIPELINE_ROOT=reports/demo/remote_data_budget_sweep_batch4 REMOTE_SCRIPT=scripts/run_remote_data_budget_sweep.sh EXTRA_PIPELINE_ARGS='RUN_SWEEP=1 PUBLISH_SWEEP_ARTIFACTS=1 TRAIN_BUDGETS=128,256,512,1024 OUTPUT_ROOT=reports/data_budget_sweep_remote_batch4' GIT_REF=codex/p1-data-budget-sweep AUTO_SHUTDOWN=0 DRY_RUN=0 bash scripts/launch_remote_medium_vast.sh`.

P1 data-budget sweep CI checkpoint (2026-06-11):
- Re-audited the current branch and external state: branch `codex/p1-data-budget-sweep` was clean except untracked `.codex/`; Vast offer search still worked, but a second clean replacement launch with the recorded safe batch-4 command again failed before instance creation with `failed with error 400: Your account lacks credit; see the billing page.`
- Opened draft PR #84 (`https://github.com/emgun/universal_simulator/pull/84`) to get the branch onto the GitHub CI path while keeping the experiment marked incomplete. PR body records the credit blocker and that no held-out/test split was touched.
...
P1 data-budget sweep resumed launch check 2 (2026-06-18):
...
P1 data-budget sweep resumed launch checkpoint (2026-06-20):
- Revalidated the branch before launch: `docs/research/p1_data_budget_sweep_contract.json` parses, `scripts/run_remote_data_budget_sweep.sh` passes `bash -n`, and `git diff --check` passed.
- PR #84 remains open as a draft at head `f4f292a124d362c68acf8ad3e9df4602c652e262`. GitHub did not attach a check run to this worklog-only head, while the last code-changing CI run remained green at `40ad3bf87408a98600c2ef5724fec414f2843244`.
- Retried the recorded safe launch command after the account-credit blocker cleared. Vast selected RTX 4090 offer `41175210`, created instance `41875102`, and the patched launcher returned `LAUNCH_RC=0`.
- The instance progressed from provisioning to active remote execution. Logs showed package setup, train/val-only hydration for `burgers1d`, `advection1d`, and `darcy2d` into `data/pdebench_medium_v1_runtime` with no test-split fetches, then started the first budget run `training-operator-ups_medium_data_budget_n128`.
- Current state is live-but-not-complete: no four-budget `data_budget_sweep_summary.json` has been verified, no B2 data-budget tarball has been verified, and no held-out/test split was touched. Continue by monitoring instance `41875102` through all four budgets, then verify OOM count, summary generation, and B2 publication before any scientific decision.

P1 data-budget sweep complete (2026-06-20):
- The full validation-only fixed-tier_b data-budget sweep completed on Vast instance `41875102` at batch size 4 with zero observed OOM/skipped-batch hits and no held-out/test data access.
- Final validation `decoded_rollout_nrmse`: `n128=0.7905242942784613`, `n256=0.8741901416127125`, `n512=0.7992866926647781`, `n1024=0.8170613903364236`. Best run is `n128`, which is worse than tier_b capacity `0.7449043873888164` and far worse than persistence `0.38260034902058476`.
- The runner hit a post-training summarizer import bug (`ModuleNotFoundError: No module named 'scripts'`) because `scripts/summarize_data_budget_sweep.py` imports `scripts.summarize_recipe_sweep` when invoked as a file without `PYTHONPATH=.`. The completed training outputs were salvaged with `PYTHONPATH=.` and the runner was patched to set `PYTHONPATH=.` for future summary calls.
- Published and verified artifact: `b2://pdebench/remote-runs/data-budget-sweep/data_budget_sweep_medium-v1_20260621T020525Z.tar.gz`, SHA256 `aebcdbf4993170b46aaa67a5421d26555fda7d979077ba00a90c6887d8ffe305`. Local B2 download matched the remote tarball hash and contained the sweep summary plus all four per-budget summaries.
- Durable artifacts added: `docs/research/artifacts/p1_data_budget_sweep_medium_v1_val.json` and `docs/research/2026-06-20-p1-data-budget-sweep-results.md`. Decision: do not promote, do not touch held-out test, stop the fixed-tier_b scale-axis line, and move next to backbone-transplant or physics-primitive paths.
