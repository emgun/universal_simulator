...
P2.1 Poseidon adapter design (2026-06-11):
- Added `docs/research/2026-06-11-p2-poseidon-adapter-design.md`, the Phase 2 transplant design doc. Grounded in the committed `poseidon_scot_val_light_v1` artifact: Poseidon-T expects 4 channels at 128x128 with lead-time conditioned layer norms, and the failed scalar attempt (`0.5453508470039229` validation) had replaced the pretrained patch embedding/recovery with fresh scalar layers.
- Design: keep the pretrained embedding/recovery frozen and adapt outside them in channel space. Option A is a replicate-initialized 1x1 channel lift/readout (~13-37 trainable params, exact pretrained behavior at init), Option B a residual shallow nonlinear lift (~500 params), Option C rank-4 LoRA under the existing Gate 2 contract (~30-80K params). All options pass true per-step lead-time conditioning and train with a one-step plus 4-step rollout objective.
- Gates unchanged: G2a validation <= 0.363424243629033; continue-zone 0.3634-0.5 unlocks B then C; above 0.5 stops Poseidon and promotes the DPOT probe. Execution checklist targets `--adapter-mode channel_lift` in the existing finetune script with replicate-init exactness and frozen-backbone unit tests, a CPU smoke, then a sub-hour GPU Gate 1 run.

P2.2 channel-lift adapter implemented (2026-06-11):
- Implemented the P2.1 design in `scripts/run_external_poseidon_scot_finetune.py`: new `--adapter-mode channel_lift` wraps the natively loaded ScOT (pretrained 4-channel patch embedding and recovery intact, `embedding_recovery_replaced=false`) with a replicate-initialized 1x1 lift and channel-mean-initialized 1x1 readout, exactly 13 trainable parameters.
- `load_poseidon_scot_model` now accepts `channels=None` to keep the checkpoint's native channel config and reports `embedding_recovery_replaced` truthfully instead of always-true.
- Added a composed rollout training loss (`--rollout-loss-steps`, `--rollout-loss-weight`) with a sequence collector, per the P1 finding that one-step-only training is the failure mode.
- Summary validator extended: channel_lift summaries must keep the pretrained embedding/recovery; the eval contract records `pretrained_embedding_recovery_intact`.
- Unit tests cover replicate-init exactness against a fake 4-channel backbone, exact 13-parameter trainable set, no backbone gradients, sequence-window shapes, rollout-loss training, and the validator guard. CPU smoke against the real hash-verified Poseidon-T checkpoint ran end-to-end (2 samples, advection1d, status complete).
- Next: the Gate 1 GPU run on light-v1 train/val (G2a threshold validation <= 0.363424243629033), blocked only on a Vast balance top-up.

Advection split regime diagnosis (2026-07-08):
- Added `scripts/diagnose_advection_split_shift_distributions.py`, a deterministic model-free protocol diagnostic that estimates per-sample transport shift via circular cross-correlation with subpixel refinement, calibrated 63/64 against official beta provenance. Reading the test shard requires an explicit `--include-test` flag and the output records `held_out_test_data_read` truthfully with `candidate_scored=false` and no ledger writes.
- Finding: the light-v1 advection splits are disjoint single-regime slices - train is entirely beta 0.1 (1.02 px/step), val entirely beta 4.0 (40.97 px/step), test entirely beta 7.0 (71.70 px/step). Medium-v1 is identical at 512/128/128 scale. Full analysis in `docs/research/2026-07-08-advection-split-regime-diagnosis.md` with artifacts under `docs/research/artifacts/`.
- This mechanically explains both held-out advection collapses (no-context model-side candidate and Poseidon channel_lift Option A), why the in-house operator never learned advection (fast transport absent from training data), why online context estimators are the only reliably transferring mechanism, and why train-fitted fixed shifts never matched validation.
- Consequences: val-selected static candidates have a structurally capped held-out ceiling under the current protocol, so Poseidon Option B/LoRA spend is paused pending a selection-protocol decision; the pending model-side beta-head pretest is the correctly shaped next measurement; universal-v1 shard prep must stratify splits by regime parameter before slicing.
- Existing claim evidence and claim language are unchanged; this is interpretation-level evidence about the frozen protocol, not a new candidate measurement.

Universal baseline experimentation roadmap (2026-07-09):
- Added `docs/superpowers/plans/2026-07-09-universal-baseline-experimentation-roadmap.md`, sequencing the path to a solid universal/general baseline in light of the split-regime diagnosis: Track A builds regime-stratified `strat-v1` shards with provenance and re-measures all baselines; Track B re-selects the Poseidon channel-lift family under honest splits; Track C makes regime handling first-class (pending beta-head pretest, parameter-conditioning interface, inferred-parameter mode); Track D gives the in-house core one confound-free retrial (its P1 advection verdict was data-confounded by beta-0.1-only training); Track E amends universal-v1 with a mandatory stratification requirement.
- Exit criteria B1-B5 define the baseline: single system beating persistence and re-measured external baselines held-out on every task with bounded per-regime spread, extrapolation reported as a separate scoped track. Estimated GPU cost to the three-task baseline: ~$5.
- The scoped beta-head held-out pretest launch was attempted and remains blocked: Vast returned `400: Your account lacks credit` (account balance reads $0); it fires with one command once credit lands.

Split integrity audit (2026-07-09):
- Added `scripts/audit_split_integrity.py` (exact byte-level cross-split overlap of initial-condition frames / full fields; requires `--include-test`; honest diagnostic flags; unit tested) and ran it on light-v1 and medium-v1.
- Findings in `docs/research/2026-07-09-split-integrity-audit.md`: Burgers held-out test is fully contained in train and identical to val (32/32 light, 128/128 medium; later frames differ only ~1e-4), advection shares all initial conditions across splits with regime-only differences, and Darcy is the only honestly disjoint task. Light-v1 therefore measures training-set reproduction (Burgers), parameter extrapolation on memorized ICs (advection), and honest generalization (Darcy only).
- Comparative claims under the frozen protocols remain internally fair (all candidates saw the same splits); absolute generalization statements should cite Darcy or wait for strat-v1. The universal-baseline roadmap Track A now marks A1 done and adds a hard build gate: zero cross-split overlap plus every-regime-in-every-split, verified and committed with shard manifests before any candidate runs.

strat-v1 advection root built and gate-verified (2026-07-09):
