...
def _write_piecewise_split(root, *, split: str, first_shift: int, second_shift: int, samples: int = 2) -> None:
...
def test_temporal_window_diagnostic_finds_common_later_shift(tmp_path):
    ...
    assert record["notes"][1] == "Scans temporal start windows; does not read or evaluate held-out test."
