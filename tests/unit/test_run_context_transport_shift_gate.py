...
from scripts.run_context_transport_shift_gate import run_gate


def _write_split(root, *, split: str, shift: int, samples: int = 2, steps: int = 6, width: int = 8) -> None:
...
def test_context_transport_gate_measures_test_after_validation_passes(tmp_path):
    ...
    assert record["estimator"]["name"] == "two_frame_context_shift"
    ...
    assert record["train"]["context_frames"] == 2
    assert record["train"]["scored_steps"] == 4
    assert record["data_sources"]["test"]["exists"] is True
    assert record["next_action"] == "held-out test measured"


def test_context_transport_gate_blocks_failed_validation_guard(tmp_path):
    _write_split(tmp_path, split="train", shift=1)
...
def test_context_transport_gate_records_and_blocks_repeat_test_measurement(tmp_path):
    _write_split(tmp_path, split="train", shift=1)
    _write_split(tmp_path, split="val", shift=2)
    _write_split(tmp_path, split="test", shift=-1)
...
def test_context_transport_gate_can_explicitly_allow_repeat_test_measurement(tmp_path):
    _write_split(tmp_path, split="train", shift=1)
    _write_split(tmp_path, split="val", shift=2)
    _write_split(tmp_path, split="test", shift=-1)
