...
from scripts.validate_ups_advection_h16_candidate_evidence import validate_evidence

ROOT = Path(__file__).resolve().parents[2]
EVIDENCE_PATH = ROOT / "docs/claim_evidence/ups_advection_h16_candidate_val_evidence.json"
...
def test_h16_candidate_evidence_validates():
    evidence = _load_current_evidence()

    assert validate_evidence(evidence, root=ROOT) == []


def test_h16_candidate_evidence_rejects_heldout_flags():
    evidence = _load_current_evidence()
...
def test_h16_candidate_evidence_recomputes_phase_gate():
    evidence = _load_current_evidence()
    mutated = copy.deepcopy(evidence)
...
def test_h16_candidate_evidence_recomputes_metrics():
    evidence = _load_current_evidence()
    mutated = copy.deepcopy(evidence)
